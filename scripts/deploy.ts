...
> {
  const equipFactory = await ethers.getContractFactory("Equip");
  ...
  const gemNesting: Nesting = await nestingFactory.deploy(
    "Gem",
    "GM",
    3000,
    pricePerMint,
  ...
  const kanariaEquip: Equip = await equipFactory.deploy(
    kanariaNesting.address,
    "fallbackURI"
  );
  const gemEquip: Equip = await equipFactory.deploy(
    gemNesting.address,
    "fallbackURI"
  );
  const base: Base = await baseFactory.deploy("KB", "svg");
...
async function setupBase(base: Base, gemAddress: string): Promise<void> {
...
async function equipGems(kanariaEquip: Equip): Promise<void> {
  const allTx = [
